...
ignored) files.
\('git add --update|--all .')."
  (interactive (progn (unless (magit-anything-staged-p)
...
(defun magit-unstage-all ()
  ...
  (interactive)
  (unless (and (not (magit-anything-unstaged-p))
...
(defun magit-reverse-files (sections args)
  (-let [(binaries sections)
    ...
    (magit-confirm-files 'reverse (mapcar #'magit-section-value sections))
    (magit-reverse-apply sections
                         (if (= (length sections) 1) (car sections) sections)
                         'magit-apply-diffs args)
    (when binaries
